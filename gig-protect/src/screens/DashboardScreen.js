...
export default function DashboardScreen({ userProfile }) {
  ...
  useEffect(() => {
    ...
    if (userProfile && userProfile?.user?.id) {
      ws = new WebSocket(
        `${getWsUrl()}/ws/telemetry/${userProfile.user.id}?token=${userProfile.user.token}`,
      );
      ...
      if (Platform.OS !== "web") {
        ...
        if (status === "granted") {
          subscription = await Location.watchHeadingAsync((headingInfo) => {
            setDeviceHeading(
              Math.round(
                headingInfo.magHeading || headingInfo.trueHeading || 0,
              ),
            );
          });
        }
    ...
    switch (activeHazard) {
      ...
      case "network":
        return {
          ...
          API: "Telecom Ping API",
        };
      case "traffic":
        return {
          color: colors.warning,
          icon: Car,
          title: "Gridlock / Accident",
      ...
      case "gate":
        return {
          color: "#A855F7",
          icon: Building,
          title: "Gate Approval Delay",
          API: "MyGate Integration",
        };
      case "power":
        return {
          color: "#F97316",
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        style={[
          styles.mapContainer,
          {
            transform: [
              {
                translateY: panelHeightAnim.interpolate({
                  inputRange: [0, 1],
                  outputRange: [0, -80],
                }),
              },
      ...
      >
        {Platform.OS !== "web" ? (
          <MapView
            ...
            initialRegion={{
              latitude: showSimulatedRoute
                ? simulatedRoute[1].latitude
                : baseLat,
              longitude: showSimulatedRoute
                ? simulatedRoute[1].longitude
                : baseLng,
              latitudeDelta: 0.015,
        ...
        <View style={styles.radarCenter} pointerEvents="none">
          <Animated.View
            style={[
              styles.radarRing,
              {
                borderColor: activeHazard
                  ? activeConfig.color
                  : colors.primaryMuted,
                transform: [
                  {
                    scale: mapPulseAnim.interpolate({
                      inputRange: [0, 1],
                      outputRange: [0.5, 3],
                    }),
                  },
                ],
                opacity: mapPulseAnim.interpolate({
                  inputRange: [0, 0.8, 1],
                  outputRange: [0.8, 0, 0],
                }),
              },
        ...
        <View style={styles.hudTop}>
          ...
          >
            <View style={styles.hudBadge}>
              ...
              <Text style={styles.hudText}>
                {userProfile?.zone?.toUpperCase() || "KORAMANGALA"} SECURED
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <TouchableOpacity
                onPress={() => setShowSimulatedRoute(!showSimulatedRoute)}
                style={[
                  styles.hudBadge,
                  { marginLeft: 10, paddingHorizontal: 10 },
                ]}
              >
                <Map
                  color={showSimulatedRoute ? colors.primary : colors.text}
                  size={16}
                />
              </TouchableOpacity>
              <TouchableOpacity
          ...
          >
            ...
            >
              ...
              >
                <Activity color={colors.success} size={12} />
                <Text
                  style={{ fontSize: 10, color: colors.success, marginLeft: 4 }}
                >
                  +12% vs avg
                </Text>
              </View>
            ...
            >
              ...
              >
                ...
                <Text
                  style={{ fontSize: 10, color: colors.primary, marginLeft: 4 }}
                >
                  2 hrs remaining
                </Text>
              </View>
        ...
        {activeHazard && (
          <View
            style={[styles.targetLock, { borderColor: activeConfig.color }]}
          >
            ...
            <View
              style={[styles.scanLine, { backgroundColor: activeConfig.color }]}
            />
            <Text style={[styles.targetText, { color: activeConfig.color }]}>
              DETECTED
            </Text>
          </View>
        )}
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.statBox}>
                ...
                <Text
                  style={[
                    styles.statValue,
                    activeHazard && { color: colors.warning },
                  ]}
                >
            ...
            <View style={styles.diagnosticPanel}>
              <View style={styles.diagnosticHeader}>
                <Text style={styles.diagnosticTitle}>
                  DEV MODE: API SIMULATION TRIGGERS
                </Text>
                <Lock color={colors.textMuted} size={14} />
              </View>
              <Text style={styles.diagnosticDesc}>
                Inject mock payloads to test the Zero-Touch Claims Engine.
              </Text>

              <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
                contentContainerStyle={styles.triggerGrid}
              >
                {[
                  {
                    type: "rain",
                  ...
                  >
                    ...
                    <Text style={[styles.triggerLabel, { color: btn.color }]}>
                      {btn.label}
                    </Text>
                  </TouchableOpacity>
                ))}
      ...
      {claimStatus && (
        <Animated.View
          style={[styles.claimOverlay, { opacity: overlayOpacityAnim }]}
        >
          <BlurView
            intensity={colors.isDark ? 40 : 80}
            tint={colors.isDark ? "dark" : "light"}
            style={StyleSheet.absoluteFillObject}
          />
          ...
          <Animated.View
            style={[
              styles.claimBoxWrapper,
              {
                transform: [
                  { translateY: overlaySlideAnim },
              ...
              {claimStatus === "processing" ? (
                <>
                  ...
                  {autoClaimReason ? (
                    ...
                    <View style={styles.verifyingList}>
                      <Text style={styles.verifyingItem}>
                        • Fetching latest endpoint data...
                      </Text>
                      <Text style={styles.verifyingItem}>
                        • Triangulating GPS coordinates...
                      </Text>
                      <Text style={styles.verifyingItem}>
                        • Matching policy bounds...
                      </Text>
                    </View>
                  )}
                  <Text style={styles.processingWait}>
                    Please wait, verifying parameters.
                  </Text>
                </>
              ) : (
                <>
                  ...
                  <View style={styles.receiptBox}>
                    ...
                    <View style={styles.receiptRow}>
                      <Text style={styles.receiptLabel}>Validation:</Text>
                      <Text style={styles.receiptValue}>
                        {activeConfig.API}
                      </Text>
                    </View>
                    <View style={styles.receiptRow}>
                      <Text style={styles.receiptLabel}>Disbursal:</Text>
                      <Text style={styles.receiptValue}>Instant Razorpay</Text>
                    </View>
...
const getStyles = (colors) =>
  StyleSheet.create({
    ...
    hudBadge: {
      flexDirection: "row",
      alignItems: "center",
      backgroundColor: "hsla(220, 78%, 76%, 0.1)",
